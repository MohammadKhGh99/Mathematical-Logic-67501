...
def prove_and_commutativity() -> Proof:
    ...
    line = Proof.Line(Formula.parse("(p&q)"))
    statement = InferenceRule([Formula.parse("(p&q)")], Formula.parse("(q&p)"))
    rules = {A_RULE, AE1_RULE, AE2_RULE}
    lines = [line, Proof.Line(Formula.parse('q'), AE1_RULE, [0]), Proof.Line(Formula.parse('p'), AE2_RULE, [0]),
...
def prove_I0() -> Proof:
    ...
    # D = D.specialize({'p':Formula.parse('p'), 'q':Formula.parse("(p->p)"), 'r':Formula.parse('p')})
    rules = {MP, I1, D}
    statement = InferenceRule([], Formula.parse("(p->p)"))
    l0 = Proof.Line(Formula.parse("((p->((p->p)->p))->((p->(p->p))->(p->p)))"), D, [])
    l1 = Proof.Line(Formula.parse("(p->((p->p)->p))"), I1, [])
    l2 = Proof.Line(Formula.parse("((p->(p->p))->(p->p))"), MP, [1, 0])
    l3 = Proof.Line(Formula.parse("(p->(p->p))"), I1, [])
    l4 = Proof.Line(Formula.parse("(p->p)"), MP, [3, 2])
...
def prove_hypothetical_syllogism() -> Proof:
    ...
    assumptions_lines = [Proof.Line(HS.assumptions[0]), Proof.Line(HS.assumptions[1])]
    l0 = Proof.Line(I1.conclusion.substitute_variables({'q': assumptions_lines[1].formula}), I1, [])
    l1 = Proof.Line(I1.conclusion.substitute_variables({'q': assumptions_lines[1].formula}).second, MP, [1, 2])
    l2 = Proof.Line(D.conclusion, D, [])
    l3 = Proof.Line(D.conclusion.second, MP, [3, 4])
    l4 = Proof.Line(D.conclusion.second.second, MP, [0, 5])
    return Proof(HS, {MP, I1, I0, D}, assumptions_lines + [l0, l1, l2, l3, l4])
