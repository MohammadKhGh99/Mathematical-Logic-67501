...
def test_propositional_skeleton(debug=False):
    ...
    for s, expected, expected_map in [
        ...
        ['(Ax[P(x)]&Ax[P(x)])', '(z5&z5)', {'z5': Formula.parse('Ax[P(x)]')}],
        ['(0=0&1=1)', '(z6&z7)', {'z6': Formula.parse('0=0'), 'z7': Formula.parse('1=1')}],
        ['((R(0)|R(1))&~R(0))', '((z8|z9)&~z8)', {'z8': Formula.parse('R(0)'), 'z9': Formula.parse('R(1)')}]
    ]:
        skeleton, substitution_map = Formula.parse(s).propositional_skeleton()
        if debug:
