...
def test_not_exists_not_implies_all_proof(debug=False):
    from predicates.some_proofs import _not_exists_not_implies_all_proof
    ...
    for formula, variable, equivalence in [
        ...
        if debug:
            print('Testing _not_exists_not_implies_all_proof to prove',
                  equivalence)
        proof = _not_exists_not_implies_all_proof(formula, variable, debug)
...
def test_exists_not_implies_not_all_proof(debug=False):
    from predicates.some_proofs import _exists_not_implies_not_all_proof
    ...
    for formula, variable, equivalence in [
        ...
        formula = Formula.parse(formula)
        if debug:
            print('Testing _exists_not_implies_not_all_proof to prove',
                  equivalence)
        proof = _exists_not_implies_not_all_proof(formula, variable, debug)
        assert proof.assumptions == Prover.AXIOMS
        assert str(proof.conclusion) == equivalence
        assert proof.is_valid()
