...
def test_instantiate_helper(debug=False):
    for formula, templates, constant_and_variable_instantiation_map, \
        ...
        ('(Ax[R(x)]->Ay[R(y)])', {'x', 'y'},
         {'x': Term('y'), 'y': Term('z')}, {}, '(Ay[R(y)]->Az[R(z)])'),
        ('(Ax[R(x)]->Ay[R(y)])', {'x', 'y'},
         {'x': Term('y'), 'y': Term('z')}, {'R': Formula.parse('R(3,_)')},
         '(Ay[R(3,y)]->Az[R(3,z)])'),
        ('(Axy[R(xy)]->R(c))', {'xy', 'c', 'R'},
         {'xy': Term('z'), 'c': Term('z')},
    ...
    for formula, templates, constant_and_variable_instantiation_map, \
        ...
        ('(Ax[(Q()->R(x))]->(Q()->Ax[R(x)]))', {'R', 'Q', 'x'}, {},
         {'Q': Formula.parse('x=0')}, 'x', 'Q'),
        ('(Ax[(Q()->R(x))]->(Q()->Ax[R(x)]))', {'R', 'Q', 'x'},
         {'x': Term('z')}, {'Q': Formula.parse('z=0')}, 'z', 'Q')]:
        formula = Formula.parse(formula)
        schema = Schema(formula, frozenset(templates))
        if debug:
            print('Substituting constant and variable instantiation map',
...
def test_instantiate(debug=False):
    for formula, templates, instantiation_map, instance in [
        ...
        ('(Ax[R(x)]->Ay[R(y)])', {'x', 'y'}, {'x': 'y', 'y': 'x'},
         '(Ay[R(y)]->Ax[R(x)])'),
        ('(Ax[R(x)]->Ay[R(y)])', {'x', 'y'}, {'x': 'y', 'y': 'z'},
         '(Ay[R(y)]->Az[R(z)])'),
        ('(Ax[R(x)]->Ay[R(y)])', {'x', 'y', 'R'},
         {'x': 'y', 'y': 'z', 'R': Formula.parse('R(3,_)')},
         '(Ay[R(3,y)]->Az[R(3,z)])'),
...
def test_prove_from_skeleton_proof(debug=False):
    ...
    from predicates.proofs import _prove_from_skeleton_proof
    ...
    for formula, skeleton, substitution_map in [
        ...
        ('(~~Ex[y=2]->Ex[y=2])', '(~~z28->z28)', {'z28': Formula.parse('Ex[y=2]')}),

        ('(Ex[Ey[x=y]]->~~Ex[Ey[x=y]])', '(z29->~~z29)', {'z29': Formula.parse('Ex[Ey[x=y]]')}),
