...
def test_combine_contradictions(debug=False):
    ...
    common_assumptions = {schema, Schema(Formula.parse('Ex[~R(x)]')), Schema(Formula.parse('Ax[R(x)]'))}
    ...
    assert is_tautology(Formula('~', combined.conclusion).propositional_skeleton()[0])
    if debug:
        print('Verifying returned proof (' +
