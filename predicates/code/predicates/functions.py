...
def reflexivity():
    args = Term(next(fresh_variable_name_generator))
    return Formula('A', str(args), Formula('SAME', [args, args]))


def symmetry():
    args = [Term(next(fresh_variable_name_generator)) for i in range(2)]
...
def transitivity():
    args = [Term(next(fresh_variable_name_generator)) for i in range(3)]
...
def relations_SAME(relation_name, arity):
    ...
    args1 = [Term(next(fresh_variable_name_generator)) for i in range(arity)]
    args2 = [Term(next(fresh_variable_name_generator)) for i in range(arity)]
