...
def test_replace_functions_with_relations_in_model(debug=False):
    model = Model(
        {'a', 'b'}, {'a': 'a'}, {'GT': {('b', 'a')}},
        {'f': {('a',): 'b', ('b',): 'b'}, 'gg': {('a', 'a'): 'b', ('a', 'b'): 'a', ('b', 'a'): 'a', ('b', 'b'): 'b'}})
    if debug:
...
def test_replace_relations_with_functions_in_model(debug=False):
    ...
    new_model = replace_relations_with_functions_in_model(model, frozenset({'f', 'gG'}))
    if debug:
        print('... got', new_model)
    assert new_model.universe == model.universe
    assert new_model.constant_meanings.keys() == {'a'}
    ...
    new_model = replace_relations_with_functions_in_model(model, frozenset({'f'}))
    if debug:
        print('... got', new_model)
    assert new_model.universe == model.universe
    assert new_model.constant_meanings.keys() == {'a'}
...
def test_compile_term(debug=False):
    ...
    for s, expected in [
        ...
        ['f(g(x,h(0)),f(f(0,g(y)),h(h(x))))',
         ['z3=h(0)', 'z4=g(x,z3)', 'z5=g(y)', 'z6=f(0,z5)', 'z7=h(x)', 'z8=h(z7)', 'z9=f(z6,z8)', 'z10=f(z4,z9)']],
