...
class Prover:
    ...
    def add_instantiated_assumption(self, instance: Union[Formula, str], assumption: Schema,
                                    instantiation_map: InstantiationMap) -> int:
        ...
        return self._add_line(Proof.AssumptionLine(instance, assumption, instantiation_map))
    ...
    def add_assumption(self, unique_instance: Union[Formula, str]) -> int:
        ...
        return self.add_instantiated_assumption(unique_instance, Schema(unique_instance), {})
    ...
    def add_mp(self, consequent: Union[Formula, str],
        ...
        return self._add_line(Proof.MPLine(consequent, antecedent_line_number, conditional_line_number))

    def add_ug(self, quantified: Union[Formula, str], unquantified_line_number: int) -> int:
        """Appends to the proof being created by the current prover a line that
        ...
        return self._add_line(Proof.UGLine(quantified, unquantified_line_number))
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str], line_number: int,
                                    term: Union[Term, str]) -> int:
        """Appends to the proof being created by the current prover a sequence
        of validly justified lines, the formula of which validly justifies the
        ...
        assert instantiation == quantified.predicate.substitute({quantified.variable: term})
        # Task 10.1

        formula = self._lines[line_number].formula
        # formula = self._lines[-1].formula
        predicate = formula.predicate.substitute({str(formula.variable): Term.parse('_')})
        sub_map = {'R': predicate, 'x': formula.variable, 'c': term}

        instantiated = self.UI.instantiate(sub_map)
        add_instantiated = self.add_instantiated_assumption(instantiated, self.UI, sub_map)
        return self.add_mp(instantiation, line_number, add_instantiated)

    def add_tautological_implication(self, implication: Union[Formula, str], line_numbers: AbstractSet[int]) -> int:
        """Appends to the proof being created by the current prover a sequence
        of validly justified lines, the last_num of which validly justifies the
        ...
        if type(implication) is str:
            implication = Formula.parse(str(implication))
        for i in line_numbers[::-1]:
            implication = Formula('->', self._lines[i].formula, implication)
        # implication = [(implication = Formula('->', self._lines[i].formula, implication)) for i in line_numbers[::-1]]
        last_num = self.add_tautology(implication)
        if len(line_numbers) > 0:
            for i in line_numbers:
                last_num = self.add_mp(self._lines[-1].formula.second, i, last_num)
        return last_num
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        first_line, second_line = self._lines[line_number1].formula, self._lines[line_number2].formula
        first_var = first_line.variable
        ug_str = 'A' + str(first_var) + '[' + str(second_line) + ']'
        ug_adding = self.add_ug(ug_str, line_number2)
        predicate = second_line.first.substitute({str(first_line.variable): Term.parse('_')})
        sub_map = {'R': predicate, 'x': first_var, 'Q': consequent}
        ug_str = str(self._lines[ug_adding].formula)
        instantiate_str = '((' + ug_str + '&' + str(first_line) + ')->' + str(second_line.second) + ')'
        instantiate_adding = self.add_instantiated_assumption(instantiate_str, self.ES, sub_map)
        line_numbers = {line_number1, ug_adding, instantiate_adding}
        return self.add_tautological_implication(second_line.second, line_numbers)

    def add_flipped_equality(self, flipped: Union[Formula, str], line_number: int) -> int:
        """Appends to the proof being created by the current prover a sequence
        of validly justified lines, the last of which validly justifies the
        ...
        cur_line = self._lines[line_number]
        first, second = cur_line.formula.arguments[:2]
        me_map = {'R': Formula('=', [Term.parse('_'), first]), 'c': first, 'd': second}
        instantiated_me = self.ME.instantiate(me_map)
        instantiated_num = self.add_instantiated_assumption(instantiated_me, self.ME, me_map)
        mp_adding = self.add_mp(self._lines[instantiated_num].formula.second, line_number, instantiated_num)

        rx_map = {'c': first}
        instantiated_rx = self.RX.instantiate(rx_map)
        instantiated_num = self.add_instantiated_assumption(instantiated_rx, self.RX, rx_map)
        return self.add_mp(self._lines[mp_adding].formula.second, instantiated_num, mp_adding)

    def add_free_instantiation(self, instantiation: Union[Formula, str], line_number: int,
                               substitution_map: Mapping[str, Union[Term, str]]) -> int:
        """Appends to the proof being created by the current prover a sequence
        of validly justified lines, the last of which validly justifies the
        ...
        cur_line = self._lines[line_number]
        sub_map = {v: Term.parse(next(fresh)) for v in substitution_map.keys()}
        formula = cur_line.formula.substitute(sub_map)
        for v in substitution_map.keys():
            line_number = self.add_ug('A' + v + '[' + str(self._lines[line_number].formula) + ']', line_number)
            t = substitution_map[v]  # ['z1': val('x')]
            formula = formula.substitute({str(sub_map[v]): Term.parse(t) if type(t) is str else t})
            temp_map = {str(_v): Term.parse(k) for k, _v in sub_map.items()}
            instantiation_formula = formula.substitute(temp_map)
            line_number = self.add_universal_instantiation(instantiation_formula, line_number, substitution_map[v])
        return line_number

    def add_substituted_equality(self, substituted: Union[Formula, str], line_number: int,
        ...
        assert substituted == Formula('=', [parametrized_term.substitute({'_': equality.arguments[0]}),
                                            parametrized_term.substitute({'_': equality.arguments[1]})])
        # Task 10.8

        first, second = self._lines[line_number].formula.arguments[:2]
        substituted_first = parametrized_term.substitute({'_': first})
        # substituted_second = parametrized_term.substitute({'_': second})
        sub_formula = Formula('=', [substituted_first, parametrized_term])
        sub_map = {'R': sub_formula, 'c': first, 'd': second}
        me_instantiated = self.ME.instantiate(sub_map)
        me_adding = self.add_instantiated_assumption(me_instantiated, self.ME, sub_map)
        mp_adding = self.add_mp(me_instantiated.second, line_number, me_adding)
        temp_map = {'c': substituted_first}
        rx_instantiated = self.RX.instantiate(temp_map)
        rx_adding = self.add_instantiated_assumption(rx_instantiated, self.RX, temp_map)
        return self.add_mp(substituted, rx_adding, mp_adding)

    def _add_chaining_of_two_equalities(self, line_number1: int, line_number2: int) -> int:
        """Appends to the proof being created by the current prover a sequence
        ...
        line1, line2 = self._lines[line_number1], self._lines[line_number2]
        to_flip = Formula('=', line1.formula.arguments[::-1])
        flippy_adding = self.add_flipped_equality(to_flip, line_number1)
        the_change = Formula('=', [Term.parse('_'), line2.formula.arguments[1]])
        sub_map = {'R': the_change, 'c': line2.formula.arguments[0], 'd': line1.formula.arguments[0]}
        me_instantiated = self.ME.instantiate(sub_map)
        me_adding = self.add_instantiated_assumption(me_instantiated, self.ME, sub_map)
        mp_formula = self._lines[me_adding].formula.second
        mp_adding = self.add_mp(mp_formula, flippy_adding, me_adding)
        mp_formula = self._lines[mp_adding].formula.second
        return self.add_mp(mp_formula, line_number2, mp_adding)
    ...
    def add_chained_equality(self, chained: Union[Formula, str],
        ...
        for i in range(1, len(line_numbers)):
            line_number = self._add_chaining_of_two_equalities(line_number, line_numbers[i])
        return line_number
