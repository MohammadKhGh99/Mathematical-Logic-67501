...
def to_not_and_or(formula: Formula) -> Formula:
    ...
    return formula.substitute_operators({"+": Formula.parse("~((p&q)|(~p&~q))"), "->": Formula.parse("(~p|q)"),
                                         "-|": Formula.parse("~(p|q)"), "<->": Formula.parse("((p&q)|(~p&~q))"),
                                         "-&": Formula.parse("~(p&q)"), "F": Formula.parse("(p&~p)"),
                                         "T": Formula.parse("(p|~p)")})
...
def to_nand(formula: Formula) -> Formula:
    ...
    return to_not_and_or(formula).substitute_operators({"~": Formula.parse("(p-&p)"),
...
def to_implies_not(formula: Formula) -> Formula:
    ...
    return to_not_and(formula).substitute_operators({"&": Formula.parse("~((~p->~q)->~q)")})
