...
def test_rule_nonsoundness_from_specialization_nonsoundness(debug=False):
    ...
    specialization = InferenceRule([Formula.parse('(x&~y)'), Formula.parse('~w')], Formula.parse('((x&~y)&~(p->z))'))
    model = frozendict({'x': True, 'y': False, 'w': False, 'p': False, 'z': True})
    ...
    assert not evaluate_inference(general, rule_nonsoundness_from_specialization_nonsoundness(general,
