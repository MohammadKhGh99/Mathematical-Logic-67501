...
def test_prove_in_model(debug=False):
    for (f, m, a, cp) in [
        ('x', {'x': True}, ['x'], ''),  # todo DONE
        ('x', {'x': False}, ['~x'], '~'),  # todo DONE
        ('~x', {'x': False}, ['~x'], ''),  # todo DONE
        ('~x', {'x': True}, ['x'], '~'),  # todo DONE
        ('x', {'x': True, 'z5': False}, ['x', '~z5'], ''),  # todo DONE
        ('(p->~p)', {'p': True}, ['p'], '~'),  # todo DONE
        ('(p->~p)', {'p': False}, ['~p'], ''),  # todo DONE
        ('(p->q)', {'p': True, 'q': True}, ['p', 'q'], ''),  # todo DONE
        ('(p->q)', {'p': True, 'q': False}, ['p', '~q'], '~'),  # todo DONE
        ('(p->q)', {'p': False, 'q': True}, ['~p', 'q'], ''),  # todo DONE
        ('(p->q)', {'p': False, 'q': False}, ['~p', '~q'], ''),  # todo DONE
        ('~~~~y7', {'y7': True}, ['y7'], ''),  # todo DONE
        ('~~~~y7', {'y7': False}, ['~y7'], '~'),  # todo DONE
        ('~(~p->~q)', {'p': True, 'q': True}, ['p', 'q'], '~'),  # todo DONE
        ('~(~p->~q)', {'p': False, 'q': True}, ['~p', 'q'], ''),  # todo DONE
        ('((p1->p2)->(p3->p4))',
         {'p1': True, 'p2': True, 'p3': True, 'p4': True},
         ['p1', 'p2', 'p3', 'p4'], ''),  # todo DONE
        ('((p1->p2)->(p3->p4))',
...
def test_prove_tautology(debug=False):
    for f, m in [
                 ('(p->p)', {'p': True}),  # todo DONE
                 ('(p->p)', {'p': False}),  # todo DONE
                 ('(p->p)', {}),  # todo DONE
                 ('((~q->~p)->(p->q))', {'p': True, 'q': False}),  # todo DONE
                 ('((~q->~p)->(p->q))', {'p': False}),  # todo DONE
                 ('((~q->~p)->(p->q))', {})  # todo DONE
                ]:
        f = Formula.parse(f)
        if debug:
    ...
    for t in [
              ...
              '(p->~~p)',  # todo DONE
              '((~p->~q)->((p->~q)->~q))',  # todo DONE
              # '((p1->(p2->(p3->p4)))->(p3->(p2->(p1->p4))))',  # todo NO
              '((p2->(p3->p4))->(p3->(p2->p4)))',  # todo DONE
              # '(((((p->q)->(~r->~s))->r)->t)->((t->p)->(s->p)))',  # todo NO
              # '(((((r->q)->(~r->~q))->r)->t)->((t->r)->(q->r)))',  # todo NO
              '(~~~~x13->~~x13)'  # todo DONE
             ]:
        t = Formula.parse(t)
        if debug:
