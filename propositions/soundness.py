...
def rule_nonsoundness_from_specialization_nonsoundness(general: InferenceRule, specialization: InferenceRule,
                                                       model: Model) -> Model:
    ...
    my_map = general.specialization_map(specialization)
    new_map = dict()
    for item in my_map:
        new_map[item] = evaluate(my_map[item], model)
    return new_map
...
def nonsound_rule_of_nonsound_proof(proof: Proof, model: Model) -> \
    ...
    for i in range(len(proof.lines)):
        if evaluate(proof.lines[i].formula, model):
            pass
        else:
            num = i
            break
    non_sound = proof.lines[num].rule
    special = proof.rule_for_line(num)
    return non_sound, rule_nonsoundness_from_specialization_nonsoundness(non_sound, special, model)
