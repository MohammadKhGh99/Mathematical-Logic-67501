...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if is_equality(formula.root):
            ...
            if formula.root in relations_instantiation_map.keys():
                new_formula = formula.substitute(constants_and_variables_instantiation_map, set())

                try:
                    substitution_map = dict()
                    for arg in new_formula.arguments:  # we got 1 argument top
                        substitution_map['_'] = arg
                    new_formula = relations_instantiation_map[new_formula.root].substitute(substitution_map, set())
                except ForbiddenVariableError:
                    # todo how the hell i can get an element from frozen set
                    raise Schema.BoundVariableError(set(bound_variables).pop(),
                                                    formula.root)  # todo [x for x in bound_variables].pop()

                free_vars = relations_instantiation_map[formula.root].free_variables()
                bound_vars = (free_vars.difference('_')).intersection(bound_variables)
                if bound_vars:
                    raise Schema.BoundVariableError(bound_vars.pop(), formula.root)
                return new_formula
            ...
            new_first = Schema._instantiate_helper(
                formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            ...
            new_second = Schema._instantiate_helper(
                formula.second, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            return Formula(formula.root, new_first, new_second)

        elif is_quantifier(formula.root):
            new_variable = Term(formula.variable).substitute(constants_and_variables_instantiation_map, set())
            new_set = set()
            new_set.add(new_variable.root)
            new_bound_variables = new_set.union(bound_variables)
            new_predicate = Schema._instantiate_helper(
                formula.predicate, constants_and_variables_instantiation_map, relations_instantiation_map,
                new_bound_variables)
            return Formula(formula.root, new_variable.root, new_predicate)
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        for key, value in instantiation_map.items():
            if is_constant(key) or is_variable(key):
                if key in self.templates:
                    ...
                    constants_and_variables_instantiation_map.update({key: value})
                else:
                    return None

            elif is_relation(key):
                if key in self.templates:
                    relations_instantiation_map.update({key: value})
                else:
                    return None
            else:
                return None
        ...
        try:
            return Schema._instantiate_helper(
                self.formula, constants_and_variables_instantiation_map, relations_instantiation_map, set())
...
class Proof:
    ...
    class AssumptionLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            for assume in assumptions:
                if str(assume.instantiate(cur_line.instantiation_map)) == str(cur_line.formula):
                    return True
            return False
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            cur_line = lines[line_number]
            if cur_line.conditional_line_number > line_number or cur_line.antecedent_line_number > line_number:
                return False
            antecedent = lines[cur_line.antecedent_line_number].formula
            conditional = lines[cur_line.conditional_line_number].formula
            if '(' + str(antecedent) + '->' + str(cur_line.formula) + ')' == str(conditional):
                return True
            return False
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            cur_line = lines[line_number]
            if cur_line.predicate_line_number > line_number:
                return False
            predicate = lines[cur_line.predicate_line_number].formula
            to_check = str(cur_line.formula)
            if to_check[0] != 'A':
                return False
            i = to_check.index('[')
            if not is_variable(to_check[1:i]):
                return False
            if str(predicate) != to_check[i + 1:-1]:
                return False
            return True
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            formula = lines[line_number].formula
            formula = formula.propositional_skeleton()[0]
...
from propositions.proofs import Proof as PropositionalProof, InferenceRule as PropositionalInferenceRule, \
    SpecializationMap as \
        PropositionalSpecializationMap
from propositions.axiomatic_systems import AXIOMATIC_SYSTEM as PROPOSITIONAL_AXIOMATIC_SYSTEM, \
...
#: schema equivalents.
PROPOSITIONAL_AXIOM_TO_SCHEMA = {I0: I0_SCHEMA, I1: I1_SCHEMA, D: D_SCHEMA,
                                 I2: I2_SCHEMA, N: N_SCHEMA, NI: NI_SCHEMA,
                                 NN: NN_SCHEMA, R: R_SCHEMA}
...
def _axiom_specialization_map_to_schema_instantiation_map(
    ...
    for k, v in propositional_specialization_map.items():
        new_map[k.capitalize()] = Formula.from_propositional_skeleton(v, substitution_map)
    return new_map


def _prove_from_skeleton_proof(formula: Formula, skeleton_proof: PropositionalProof,
                               substitution_map: Mapping[str, Formula]) -> Proof:
    ...
    for line in skeleton_proof.lines:
        ...
        if len(line.assumptions) == 2:
            lines.append(Proof.MPLine(formula, line.assumptions[0], line.assumptions[1]))
        else:
            special_map = PropositionalInferenceRule._formula_specialization_map(line.rule.conclusion, line.formula)
            new_map = _axiom_specialization_map_to_schema_instantiation_map(special_map, substitution_map)
            lines.append(Proof.AssumptionLine(formula, PROPOSITIONAL_AXIOM_TO_SCHEMA[line.rule], new_map))
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    formula, sub_map = tautology.propositional_skeleton()
    return _prove_from_skeleton_proof(tautology, prove_propositional_tautology(formula), sub_map)
